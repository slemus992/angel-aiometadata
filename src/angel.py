...
class AngelScraper:
    ...
    def _scrape_page(
        ...
        try:
            ...
            log(f"Found {len(json_items)} raw Project nodes in __NEXT_DATA__ before dedup")
    ...
    def _extract_media_nodes(self, data) -> List[Dict]:
        """Recursively find dictionaries that represent an Angel Studios Project.

        The site's embedded data currently shapes these as:
        {"id": ..., "slug": ..., "name": ..., "releaseDate": ..., "__typename": "Project", ...}
        """
        nodes = []
        if isinstance(data, dict):
            if data.get("__typename") == "Project" and "slug" in data and "name" in data:
                nodes.append(data)
